...
export const useHomeStore = create<HomeState>((set: (state: Partial<HomeState> | ((state: HomeState) => Partial<HomeState>)) => void, get: () => HomeState) => ({
    ...
    fetchRankingsData: async (token?: string | null) => {
        // 1. Try to get cached rankings first
        const cacheKey = 'home_rankings_all';
        ...
        if (cachedData) {
            logger.debug('🏆 Rankings cache hit - displaying immediately');
            set({
                videos: cachedData.videos,
                players: cachedData.players,
                teamOfMonth: cachedData.teamOfMonth,
                loadingRankings: false
            });
        ...
        try {
            logger.debug('🏆 Fetching fresh rankings data...');
            ...
            const finalData = { videos, players, teamOfMonth };
            
            // Save to cache
            await cacheService.set(cacheKey, finalData, 30 * 60 * 1000); // 30 minutes cache
