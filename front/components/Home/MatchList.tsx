    ...
    <Animated.View entering={FadeInDown.delay(index * 80).springify().damping(14)}>
      ...
      >
        ...
        <View style={styles.cardBottom}>
          <TouchableOpacity
            onPress={() => { setStarred(s => !s); onFavoritePress?.(match.id); }}
            activeOpacity={0.7}
            hitSlop={12}
...
export function MatchList({ isLoading = false, matches: matchesProp, onMatchPress, onViewAllPress, onFavoritePress }: MatchListProps) {
  const router = useRouter();
  const data = matchesProp ?? matches;
  const hasMatches = data.length > 0;
  const shimmerX = useShimmer();
  const openMatchesHub = () => (onViewAllPress ? onViewAllPress() : router.push('/matches'));
  ...
  return (
    <View style={styles.section}>
      ...
      {isLoading ? (
        ...
        >
          {data.map((m, i) => (
            <MatchCard key={m.id} match={m} index={i} onOpenHub={() => onMatchPress ? onMatchPress(m.id) : openMatchesHub()} onFavoritePress={onFavoritePress} />
          ))}
          <EmptyMatchCard />
        </ScrollView>
      ) : (
