    ...
    <TouchableOpacity style={[styles.playerNode, style]} onPress={onPress}>
        <View style={styles.nodeContent}>
            {player.image && player.image.trim() !== '' && !imageError ? (
                ...
                <View style={{ width: (styles.nodeImage as any)?.width ?? 48, height: (styles.nodeImage as any)?.height ?? 48, borderRadius: (styles.nodeImage as any)?.borderRadius ?? 24, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.1)', borderWidth: 2, borderColor: COLORS.white }}>
                    <Text style={{ color: '#fff', fontSize: 16, fontWeight: 'bold' }}>{getInitials(player.name)}</Text>
                </View>
            )}
...
export const TeamPitch = React.memo(({ players, onPlayerPress }: TeamPitchProps) => {
