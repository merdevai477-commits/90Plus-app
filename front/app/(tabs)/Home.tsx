...
export default function HomeScreen() {
    ...
    try {
      ...
      if (data.status === 'SUCCESS') {
        setUserInfo(prev => ({
          ...
          nextSpinTime: !data.data.canSpin && data.data.timeRemaining
            ? new Date(Date.now() + (data.data.timeRemaining.hours * 3600000) + (data.data.timeRemaining.minutes * 60000))
            : undefined
        }));
      }
    } catch (error) {
  ...
  const fetchUserProfile = useCallback(async () => {
    const applyClerkHomeFallback = () => {
      ...
      setUserInfo(prev => ({
        ...
        avatar: user.imageUrl || user.externalAccounts?.[0]?.imageUrl || prev.avatar
      }));
    };
    ...
    try {
      ...
      try {
        ...
        setUserInfo(prev => ({
          ...
          loginStreak: userData.consecutiveLoginDays !== undefined ? userData.consecutiveLoginDays : prev.loginStreak
        }));

        // Sync global state
        if (userData.username && globalState.userProfile) globalState.userProfile.username = userData.username;
        if (userData.avatar && globalState.userProfile) globalState.userProfile.avatar = userData.avatar;
      } catch {
  ...
  // Sync predictions state when store updates - ✅ TOP LEVEL HOOK
  useEffect(() => {
  ...
  return (
    <View style={[styles.container, { paddingTop: 0 }]}>
      ...
      >
        <WelcomeSection
          onRegisterPress={useCallback(() => router.push('/signup'), [router])}
          onLoginPress={useCallback(() => router.push('/auth'), [router])}
          onProfilePress={useCallback(() => router.navigate('/(tabs)/profile'), [router])}
          onSpinWheelPress={useCallback(() => setLuckyWheelVisible(true), [])}
          onPredictionsPress={useCallback(() => router.navigate({ pathname: '/(tabs)/matches', params: { tab: 'predictions' } }), [router])}
          onQuizPress={useCallback(() => router.navigate('/(tabs)/quiz'), [router])}
          onRankPress={useCallback(() => router.navigate('/(tabs)/rank'), [router])}
          username={userInfo.username}
          userAvatar={userInfo.avatar}
          predictionsCount={userInfo.remainingPredictions}
          streakDays={globalState.userProfile?.stats?.predictions || 0}
          userRank={userInfo.rank || 0}
          spinWheelAvailable={userInfo.spinWheelAvailable}
          nextSpinTime={userInfo.nextSpinTime}
          loginStreak={userInfo.loginStreak}
          dailyQuizStatus={userInfo.dailyQuizStatus}
        />

        <MatchList
          matches={displayMatches}
          onMatchPress={handleMatchPress}
          onViewAllPress={useCallback(() => router.navigate('/(tabs)/matches'), [router])}
        ...
        <PlayerList
          players={players}
          onPlayerPress={useCallback((player) => {
            if (player.username) {
              router.push({
