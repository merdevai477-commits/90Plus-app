...
export default function ChatScreen() {
  ...
  const getChatLabels = useCallback(
    () => ({
      initialWelcome: tChat.initialWelcome,
      streamRetry: (current: number, max: number) =>
        tChat.streamRetry.replace('{current}', String(current)).replace('{max}', String(max)),
      streamRetryFailed: tChat.streamRetryFailed,
      noActiveConversation: tChat.noActiveConversation,
      exportUserLabel: tChat.exportUserLabel,
      exportAiLabel: tChat.exportAiLabel,
    }),
    [tChat],
  );
  ...
  } = useAIChatNative({ getSystemPromptSuffix, getChatLabels });
    ...
    <ChatScreenLayout
      ...
      composer={
        <ChatComposerDock
          ...
          limitResetsAfterText={tChat.limitResetsAfter}
          stopLabel={tChat.stopGeneration}
        />
      }
    />
  );
